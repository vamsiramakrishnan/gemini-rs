...
#[derive(Parser)]
#[command(name = "gemini-adk-transpiler")]
...
enum Commands {
    ...
    /// One-shot: read ADK-JS source and generate compilable Rust code targeting gemini-adk
    Transpile {
        /// Path to the ADK-JS source directory (e.g. /tmp/adk-js/core/src/)
        #[arg(short, long)]
        source: PathBuf,

        /// Output Rust source file path (e.g. crates/gemini-adk/src/agents/generated.rs)
        #[arg(short, long)]
        output: PathBuf,
...
fn run_transpile(source: &Path, output: &Path, genai_source: Option<&Path>) {
    ...
    println!("Types: {}", schema.types.len());
    println!("Output size: {} bytes", rust_code.len());
    println!("Target: gemini-adk (compilable)");
}
