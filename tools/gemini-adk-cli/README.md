...
```bash
# From workspace root
cargo install --path tools/gemini-adk-cli

# Or run directly
cargo run -p gemini-adk-cli -- <command>
```
...
2. **`agent.json` / `root_agent.json`** — JSON format (used by `gemini-adk-api`, matches upstream ADK)
