# rs-adk: DevEx & Performance Reference
...
TextAgentTool.call(args)
  └─ LlmTextAgent.run(state)
       └─ GeminiLlm.generate(request)          ← uses cached Client
            └─ rs_genai::Client.generate_content_with()
...
- `GeminiLlm` — owns `rs_genai::Client`, created once
- `rs_genai::Client` — owns `reqwest::Client` with connection pool
...
| rs_genai Client | Owned by GeminiLlm | Session |
| reqwest Client | Owned by rs_genai Client | Session (connection pool) |
...
4. **Client reuse.** `GeminiLlm` caches `rs_genai::Client` (and its `reqwest::Client` with HTTP/2 connection pool). No per-call client creation.
...
9. **enter_prompt() hides Content import.** Users don't need `use rs_genai::prelude::Content` for the most common phase-entry pattern.
