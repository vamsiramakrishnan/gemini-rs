# Coding-agent API reference

## Architecture

Three-crate layered stack for the Gemini Multimodal Live API:

<p align="center"><img src="assets/diagrams/architecture-stack.svg" alt="Three-crate layered stack: L2 fluent DX over L1 runtime over L0 wire protocol" width="760"></p>
...
- The current fast-lane dispatcher emits transcription fragments with `is_final = false`. It does not synthesize a final callback at the turn boundary. Accumulate fragments explicitly if a consumer needs a complete transcript; do not wait indefinitely for `is_final = true`.
...
| `on_generation_complete` | `()` | Provider emitted `GenerationComplete`; see the event limits below |
...
- **`on_generation_complete`**: runs when the provider emits `GenerationComplete`. It is a lifecycle signal, not a source of output that the provider never sent. Do not assume an interrupted generation will yield the model's complete intended response.
...
under the historical model configuration used by the example);
...
<p align="center"><img src="assets/diagrams/three-lane-processor-full.svg" alt="Three-lane processor: fast, control, and telemetry lanes" width="820"></p>
