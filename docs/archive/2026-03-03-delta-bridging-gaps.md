...
- `crates/gemini-adk-rs/src/state.rs` — add `with()`, `with_raw()`
- `crates/gemini-adk-fluent-rs/src/compose/state.rs` — rewrite S predicates to use `with()`
...
- `crates/gemini-adk-rs/src/live/transcript.rs`
- `crates/gemini-adk-rs/src/live/builder.rs` — expose `.transcript_capacity(n)` on LiveSessionBuilder
...
- `crates/gemini-adk-rs/src/llm/mod.rs` — add `into_parts()`
- `crates/gemini-adk-rs/src/text.rs` — use consuming access pattern
...
**Better approach** — make `gemini_genai_rs::Client` accept a `TokenProvider` trait so it refreshes transparently:

```rust
// gemini-genai-rs/src/transport/auth.rs — extend VertexAIAuth
...
- `crates/gemini-genai-rs/src/transport/auth.rs` — `TokenProvider` trait, `GcloudTokenProvider`
- `crates/gemini-adk-rs/src/llm/gemini.rs` — accept `TokenProvider` in params
- `crates/gemini-adk-rs/src/llm/mod.rs` — re-export
...
- `crates/gemini-adk-rs/src/state.rs` — `StateKey<T>`, `get_key()`, `set_key()`, `with_key()`
- `crates/gemini-adk-fluent-rs/src/compose/state.rs` — `S::key_is_true()`, `S::key_eq()`
...
impl Live {
    ...
    where
        F: Fn(String, gemini_adk_rs::llm::LlmError) -> Fut + Send + Sync + 'static,
...
- `crates/gemini-adk-rs/src/live/callbacks.rs` — add field
- `crates/gemini-adk-rs/src/live/processor.rs` — fire callback
- `crates/gemini-adk-fluent-rs/src/live.rs` — builder method
...
```rust
// gemini-adk-fluent-rs/src/live.rs
...
- `crates/gemini-adk-fluent-rs/src/lib.rs` — `let_clone!` macro
- `apps/gemini-adk-web-rs/src/apps/*.rs` — adopt macro
...
- `crates/gemini-adk-rs/src/live/phase.rs`
...
- `crates/gemini-adk-rs/src/tool.rs`
...
- `crates/gemini-adk-fluent-rs/src/compose/tools.rs` — expose decomposition methods
- `crates/gemini-adk-fluent-rs/src/live.rs` — `with_tools()` method
...
- Option B: `crates/gemini-adk-fluent-rs/src/compose/middleware.rs` — delete or mark `#[doc(hidden)]`
...
- `crates/gemini-adk-rs/src/live/extractor.rs` — `should_extract()` default
- `crates/gemini-adk-rs/src/live/processor.rs` — filter before `join_all`
...
- `crates/gemini-adk-rs/src/text.rs` — `warm_up()` on TextAgent trait + LlmTextAgent override
- `crates/gemini-adk-fluent-rs/src/live.rs` — auto-warm in `build_and_connect()`
