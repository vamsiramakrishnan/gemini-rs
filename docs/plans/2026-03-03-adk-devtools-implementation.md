...
**Goal:** Ship two new crates — `adk-devtools` (library) and `gemini-adk-cli-rs` (binary) — providing `adk web` and `adk run` commands with a binary-multiplexed WebSocket protocol and embedded dev UI.

**Architecture:** `adk-devtools` is a library crate with protocol types, Axum DevServer, terminal REPL, and embedded vanilla JS/CSS dev UI. `gemini-adk-cli-rs` is a thin binary wrapping `cargo run` with env vars. The protocol uses binary WebSocket frames for audio and JSON text frames for control/devtools channels.
...
[dependencies]
gemini-adk-fluent-rs = { path = "../gemini-adk-fluent-rs" }
gemini-adk-rs = { path = "../gemini-adk-rs" }
gemini-genai-rs = { path = "../gemini-genai-rs" }
...
Reference the existing demo patterns from `apps/gemini-adk-web-rs/src/main.rs` and `apps/gemini-adk-web-rs/src/ws_handler.rs`, but with the binary protocol.
...
**Reference files:**
- `apps/gemini-adk-web-rs/static/js/app.js` — reuse WebSocket lifecycle, upgrade to binary protocol
- `apps/gemini-adk-web-rs/static/js/audio.js` — reuse recording/playback, upgrade to AudioWorklet + binary frames
- `apps/gemini-adk-web-rs/static/js/devtools.js` — reuse state grouping, event logging, telemetry
...
Devtools panel manager — reuse and refactor from `apps/gemini-adk-web-rs/static/js/devtools.js`:
...
## Task 8: Scaffold `gemini-adk-cli-rs` binary crate

**Files:**
- Create: `tools/gemini-adk-cli-rs/Cargo.toml`
- Create: `tools/gemini-adk-cli-rs/src/main.rs`
- Modify: `Cargo.toml` (workspace members)

**Step 1: Create `tools/gemini-adk-cli-rs/Cargo.toml`**

```toml
[package]
name = "gemini-adk-cli-rs"
version = "0.1.0"
edition.workspace = true
license.workspace = true
repository.workspace = true
...
**Step 2: Create `tools/gemini-adk-cli-rs/src/main.rs`**
...
Add `"tools/gemini-adk-cli-rs"` to the workspace members.

**Step 4: Verify**

Run: `cargo check -p gemini-adk-cli-rs`
Run: `cargo run -p gemini-adk-cli-rs -- --help`
Expected: Shows help with `web` and `run` subcommands.

**Step 5: Commit**

```bash
git add tools/gemini-adk-cli-rs/ Cargo.toml
git commit -m "feat(gemini-adk-cli-rs): scaffold CLI binary with web and run subcommands"
```
...
fn registry_register_and_list() {
    ...
    reg.register(AgentDescriptor {
        ...
        factory: std::sync::Arc::new(|| {
            gemini_adk_fluent_rs::live::Live::builder()
        }),
    });
...
| 8 | `adk` CLI binary | `tools/gemini-adk-cli-rs/` |
