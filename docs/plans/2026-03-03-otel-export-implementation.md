...
### Task 1: Add OTel dependencies to gemini-genai-rs Cargo.toml

**Files:**
- Modify: `crates/gemini-genai-rs/Cargo.toml`
...
Run: `cargo check -p gemini-genai-rs`
Expected: Compiles with no new warnings (otel deps are optional, not pulled in).

**Step 3: Verify it compiles with the feature**

Run: `cargo check -p gemini-genai-rs --features otel`
Expected: Compiles. The OTel crates are pulled in but not yet used.

**Step 4: Commit**

```bash
git add crates/gemini-genai-rs/Cargo.toml
git commit -m "feat(gemini-genai-rs): add optional otel dependencies behind feature flag"
```
...
**Files:**
- Modify: `crates/gemini-genai-rs/src/telemetry/mod.rs`
...
```bash
git add crates/gemini-genai-rs/src/telemetry/mod.rs
git commit -m "feat(gemini-genai-rs): add OTel OTLP trace and metrics export to TelemetryConfig"
```

---

### Task 3: Forward otel feature flag through gemini-adk-rs and gemini-adk-fluent-rs

**Files:**
- Modify: `crates/gemini-adk-rs/Cargo.toml`
- Modify: `crates/gemini-adk-fluent-rs/Cargo.toml`

**Step 1: Add otel feature to gemini-adk-rs**

In `crates/gemini-adk-rs/Cargo.toml`, add after the `metrics` feature (line 40):

```toml
otel = ["gemini-genai-rs/otel", "tracing-support"]
```

**Step 2: Add otel feature to gemini-adk-fluent-rs**

In `crates/gemini-adk-fluent-rs/Cargo.toml`, add after `gemini-llm` feature (line 29):

```toml
otel = ["gemini-adk-rs/otel", "gemini-genai-rs/otel"]
```
...
```bash
git add crates/gemini-adk-rs/Cargo.toml crates/gemini-adk-fluent-rs/Cargo.toml
git commit -m "feat(adk): forward otel feature flag through gemini-adk-rs and gemini-adk-fluent-rs"
```
...
- Modify: `apps/gemini-adk-web-rs/README.md`

**Step 1: Add optional otel feature to Web UI**

In `apps/gemini-adk-web-rs/Cargo.toml`, add a `[features]` section at the end:

```toml
[features]
default = []
otel = ["gemini-genai-rs/otel", "gemini-adk-rs/otel"]
```

**Step 2: Update README with OTel instructions**

In `apps/gemini-adk-web-rs/README.md`, add a new section after the "Model Routing" section documenting:
...
```bash
cargo run -p gemini-genai-ui --features otel
```
...
Run: `cargo check -p gemini-genai-ui && cargo check -p gemini-genai-ui --features otel`
Expected: Both compile.

**Step 4: Commit**

```bash
git add apps/gemini-adk-web-rs/Cargo.toml apps/gemini-adk-web-rs/README.md
...
let _telemetry_guard = gemini_genai_rs::telemetry::TelemetryConfig {
    logging_enabled: true,
...
Run: `cargo check -p gemini-genai-ui --features otel`
...
```bash
git add apps/gemini-adk-web-rs/src/main.rs
...
Run: `cargo build --workspace --features gemini-genai-rs/otel`
...
Run: `cargo test -p gemini-genai-rs --features otel`
Expected: All gemini-genai-rs tests pass with the otel feature enabled.

**Step 4: Verify no otel deps leak into default build**

Run: `cargo tree -p gemini-genai-rs -e features | grep opentelemetry`
Expected: No output (opentelemetry not in default dependency tree).

Run: `cargo tree -p gemini-genai-rs -e features --features otel | grep opentelemetry`
