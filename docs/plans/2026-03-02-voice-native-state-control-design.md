...
|-----------|-------|------------|-------------|
| Session signals | L1 (gemini-adk-rs processor) | Small | Modify processor to auto-track counters |
| State namespacing | L1 (gemini-adk-rs state) | Small | Add prefix helpers, `session()` accessor |
| `computed()` | L1 (gemini-adk-rs) + L2 (fluent) | Medium | Dependency graph, topo-sort, recompute on TurnComplete |
| Phase machine | L1 (gemini-adk-rs) + L2 (fluent) | Medium | Phase struct, transition evaluator, instruction override |
| `watch()` | L1 (gemini-adk-rs) + L2 (fluent) | Medium | Previous-value tracking, predicate evaluation |
| Temporal patterns | L1 (gemini-adk-rs) + L2 (fluent) | Medium | Event ring buffer, timer task, cooldown tracking |
| Evaluation pipeline integration | L1 (gemini-adk-rs processor) | Medium | Wire steps 3-7 into TurnComplete handler |

### 10.2 What Does NOT Change

- **gemini-genai-rs (L0)**: Zero changes. The wire protocol layer remains untouched.
