...
**Tech Stack:** Rust (axum, tokio, serde_json), rs-genai, rs-adk, adk-rs-fluent, HTML/CSS/JS (vanilla)
...
impl CookbookApp for VoiceChatApp {
    ...
    async fn handle_session(
        ...
        let mut builder = adk_rs_fluent::Live::builder()
            .model(model)
            .voice(voice)
...
**Step 4: Add `adk-rs-fluent` dependency to Cargo.toml**
...
Uses `TypedTool` + `ToolDispatcher` from rs-adk:
...
**Step 5: Add rs-adk + schemars dependencies to Cargo.toml**

```toml
rs-adk = { path = "../../crates/rs-adk" }
schemars = "0.8"
```
...
1. `cargo build -p rs-genai-ui` — compiles
2. `cargo run -p rs-genai-ui` — server starts on port 3000
