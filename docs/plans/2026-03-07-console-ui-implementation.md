...
**Goal:** Rebuild the apps/gemini-adk-web-rs devtools panel into an OTel-native trace explorer with virtualized rendering, unified timeline, canvas minimap, token/usage metrics, and a SessionBridge to eliminate app boilerplate.
...
**Files:**
- Create: `apps/gemini-adk-web-rs/static/js/render/ring-buffer.js`
- Create: `apps/gemini-adk-web-rs/static/js/render/virtual-list.js`
- Create: `apps/gemini-adk-web-rs/static/js/render/render-scheduler.js`
...
```bash
git add apps/gemini-adk-web-rs/static/js/render/
...
**Files:**
- Modify: `apps/gemini-adk-web-rs/static/app.html` — add `<script>` tags for render modules, add minimap canvas placeholder
- Rewrite: `apps/gemini-adk-web-rs/static/js/devtools.js` — new `DevtoolsManager` using rendering primitives
- Modify: `apps/gemini-adk-web-rs/static/css/devtools.css` — new timeline row styles
...
Run: `cargo run -p gemini-genai-ui` and connect to a text-chat or tool-calling app. Verify:
...
```bash
git add apps/gemini-adk-web-rs/static/js/devtools.js apps/gemini-adk-web-rs/static/app.html apps/gemini-adk-web-rs/static/css/devtools.css
...
**Files:**
- Create: `apps/gemini-adk-web-rs/static/js/render/minimap.js`
- Modify: `apps/gemini-adk-web-rs/static/js/devtools.js` — integrate minimap
- Modify: `apps/gemini-adk-web-rs/static/css/devtools.css` — minimap container styles
...
```bash
git add apps/gemini-adk-web-rs/static/js/render/minimap.js apps/gemini-adk-web-rs/static/js/devtools.js apps/gemini-adk-web-rs/static/css/devtools.css
...
**Files:**
- Create: `apps/gemini-adk-web-rs/static/js/render/sparkline.js`
- Modify: `apps/gemini-adk-web-rs/static/js/devtools.js` — new `_renderMetrics()` method
- Modify: `apps/gemini-adk-web-rs/static/css/devtools.css` — metrics panel styles
...
```bash
git add apps/gemini-adk-web-rs/static/js/render/sparkline.js apps/gemini-adk-web-rs/static/js/devtools.js apps/gemini-adk-web-rs/static/css/devtools.css
...
**Files:**
- Modify: `apps/gemini-adk-web-rs/static/js/devtools.js` — new state panel with `Map<key, DOMElement>` tracking
- Modify: `apps/gemini-adk-web-rs/static/css/devtools.css` — diff flash + search styles
...
**Files:**
- Modify: `apps/gemini-adk-web-rs/static/js/devtools.js` — new `_renderPhases()` method
- Modify: `apps/gemini-adk-web-rs/static/css/devtools.css` — phase hero and duration bar styles
...
**Files:**
- Create: `apps/gemini-adk-web-rs/src/bridge.rs`
- Modify: `apps/gemini-adk-web-rs/src/main.rs` — add `mod bridge;`
- Modify: `apps/gemini-adk-web-rs/src/apps/voice_chat.rs` — migrate to SessionBridge
- Modify: `apps/gemini-adk-web-rs/src/apps/tool_calling.rs` — migrate to SessionBridge
- Modify: `apps/gemini-adk-web-rs/src/apps/text_chat.rs` — migrate to SessionBridge
...
Run: `cargo run -p gemini-genai-ui`, test voice-chat, text-chat, and tool-calling apps.
Verify: identical behavior to before migration.

**Step 8: Commit**

```bash
git add apps/gemini-adk-web-rs/src/bridge.rs apps/gemini-adk-web-rs/src/main.rs apps/gemini-adk-web-rs/src/apps/voice_chat.rs apps/gemini-adk-web-rs/src/apps/text_chat.rs apps/gemini-adk-web-rs/src/apps/tool_calling.rs
...
**Files:**
- Modify: `apps/gemini-adk-web-rs/src/app.rs` — add `SpanEvent` and `TurnMetrics` variants to `ServerMessage`

**Step 1: Add new variants to `ServerMessage` enum**

In `apps/gemini-adk-web-rs/src/app.rs`, add after the `ToolCallEvent` variant:
...
```bash
git add apps/gemini-adk-web-rs/src/app.rs apps/gemini-adk-web-rs/static/js/devtools.js apps/gemini-adk-web-rs/static/js/app.js
...
**Files:**
- Create: `apps/gemini-adk-web-rs/src/span_layer.rs`
- Modify: `apps/gemini-adk-web-rs/src/main.rs` — add `mod span_layer;`
- Modify: `apps/gemini-adk-web-rs/src/bridge.rs` — integrate span layer into session lifecycle
- Modify: `apps/gemini-adk-web-rs/Cargo.toml` — add `tracing-subscriber` dependency

**Step 1: Add dependency**

In `apps/gemini-adk-web-rs/Cargo.toml`, add:
```toml
...
```rust
//! WebSocketSpanLayer — bridges tracing spans from gemini-genai-rs/gemini-adk-rs to the browser.
//!
//! Only captures spans with targets `gemini_genai_rs` or `gemini` (the ~13 span types
...
```bash
git add apps/gemini-adk-web-rs/src/span_layer.rs apps/gemini-adk-web-rs/src/main.rs apps/gemini-adk-web-rs/src/bridge.rs apps/gemini-adk-web-rs/Cargo.toml
...
**Files:**
- Modify: `apps/gemini-adk-web-rs/static/js/devtools.js` — add export button and OTLP JSON serialization
- Modify: `apps/gemini-adk-web-rs/static/css/devtools.css` — export button styles
...
_exportOtlpJson() {
  ...
  const otlp = {
    resourceSpans: [{
      ...
      scopeSpans: [{
        scope: { name: 'gemini-genai-ui', version: '0.1.0' },
        spans: spans.map(s => ({
...
When a `spanEvent` with name `gemini_genai_rs.session` arrives, extract and display the trace ID:

```js
// In addEvent():
if (msg.type === 'spanEvent' && msg.name === 'gemini_genai_rs.session') {
