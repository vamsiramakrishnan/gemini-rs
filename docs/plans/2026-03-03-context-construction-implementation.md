...
**Architecture:** L0 (gemini-genai-rs) is untouched. L1 (gemini-adk-rs) gains new types in `phase.rs`, a `TranscriptWindow` type in `transcript.rs`, and processor.rs is split into composable functions. L2 (gemini-adk-fluent-rs) gains new `PhaseBuilder` methods and `P` module extensions. Demos adopt per-phase modifiers replacing global `instruction_template`.

**Tech Stack:** Rust, tokio, serde_json, gemini-genai-rs (L0 wire), gemini-adk-rs (L1 runtime), gemini-adk-fluent-rs (L2 fluent)
...
**Files:**
- Modify: `crates/gemini-adk-rs/src/live/transcript.rs`
- Modify: `crates/gemini-adk-rs/src/live/mod.rs` (re-export)
...
In `crates/gemini-adk-rs/src/live/mod.rs`, change line 36:
```rust
...
Run: `cargo test -p gemini-adk-rs transcript`
Expected: All new tests pass, existing tests unaffected.

**Step 6: Commit**

```
feat(gemini-adk-rs): add TranscriptWindow for phase callback context access
```
...
**Files:**
- Modify: `crates/gemini-adk-rs/src/live/phase.rs`
- Modify: `crates/gemini-adk-rs/src/live/mod.rs` (re-export)
...
In `crates/gemini-adk-rs/src/live/mod.rs`, update line 29:
```rust
...
Run: `cargo test -p gemini-adk-rs phase`
Expected: All tests pass.

**Step 10: Commit**

```
feat(gemini-adk-rs): add InstructionModifier, TransitionResult, on_enter_context, prompt_on_enter to Phase
```
...
```
refactor(gemini-adk-rs): modularize processor.rs, unify instruction composition into single send
```
...
**Files:**
- Modify: `crates/gemini-adk-fluent-rs/src/live_builders.rs`
...
use gemini_adk_rs::live::transcript::TranscriptWindow;
```
...
```rust
    ...
    where
        F: Fn(&State, &TranscriptWindow) -> Option<Vec<gemini_genai_rs::prelude::Content>>
            + Send + Sync + 'static,
    {
...
Run: `cargo test -p gemini-adk-fluent-rs`
...
```
feat(gemini-adk-fluent-rs): add with_state, when, prompt_on_enter, on_enter_context to PhaseBuilder
```
...
**Files to check:**
- `crates/gemini-adk-rs/src/live/builder.rs` — may construct Phase
- `apps/gemini-adk-web-rs/src/apps/*.rs` — construct phases via L2 builder (should be fine)
...
Run: `cargo build -p gemini-adk-web-rs`
Expected: Clean compile.
