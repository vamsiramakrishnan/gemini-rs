...
**Files:**
- Modify: `crates/rs-genai/src/session/mod.rs` (SessionCommand enum + SessionWriter trait + SessionHandle impl)
- Modify: `crates/rs-genai/src/transport/codec.rs` (encode new commands)

**Step 1: Add SessionCommand variants**

In `crates/rs-genai/src/session/mod.rs`, add to the `SessionCommand` enum:
...
In `crates/rs-genai/src/transport/codec.rs`, add to `encode_command` match:
...
**Step 5: Update AgentSession in rs-adk**

In `crates/rs-adk/src/agent_session.rs`, add:
...
```
feat(rs-genai): add send_video and update_instruction to SessionHandle
```
...
**Files:**
- Create: `crates/rs-adk/src/live/mod.rs`
- Create: `crates/rs-adk/src/live/callbacks.rs`
- Modify: `crates/rs-adk/src/lib.rs` (add `pub mod live;`)
...
Add to `crates/rs-adk/src/lib.rs`:
```rust
pub mod live;
```

**Step 4: Add `bytes` dependency to rs-adk Cargo.toml**
...
```
feat(rs-adk): add EventCallbacks typed callback registry for Live sessions
```
...
**Files:**
- Create: `crates/rs-adk/src/live/processor.rs`
- Modify: `crates/rs-adk/src/live/mod.rs`
...
**Step 3: Add dependencies to rs-adk Cargo.toml**

Add `bytes`, `parking_lot`, `arc-swap`, `tracing` (if not already present).

**Step 4: Run tests**

Run: `cargo test -p rs-adk && cargo build -p rs-adk`

**Step 5: Commit**

```
feat(rs-adk): add two-lane event processor for Live sessions
```
...
**Files:**
- Create: `crates/rs-adk/src/live/builder.rs`
- Create: `crates/rs-adk/src/live/handle.rs`
- Modify: `crates/rs-adk/src/live/mod.rs`

**Step 1: Create LiveHandle**

`crates/rs-adk/src/live/handle.rs` — runtime interaction with a live session:
...
use rs_genai::prelude::{Content, FunctionResponse, SessionEvent, SessionPhase};
...
`crates/rs-adk/src/live/builder.rs` — builds and connects a Live session:
...
use rs_genai::prelude::SessionConfig;
use rs_genai::session::SessionHandle;
...
impl LiveSessionBuilder {
    ...
    pub async fn connect(self) -> Result<LiveHandle, AgentError> {
        // Connect via L0
        let session: SessionHandle = rs_genai::connect(self.config)
            .await
            .map_err(AgentError::Session)?;

        // Wait for Active phase
        session
            .wait_for_phase(rs_genai::prelude::SessionPhase::Active)
            .await;

        let callbacks = Arc::new(self.callbacks);
        let writer: Arc<dyn rs_genai::session::SessionWriter> = Arc::new(session.clone());
...
In `crates/rs-adk/src/lib.rs`, add to the public API:
```rust
...
Run: `cargo build -p rs-adk && cargo test -p rs-adk`

**Step 7: Commit**

```
feat(rs-adk): add LiveSessionBuilder and LiveHandle for callback-driven sessions
```
...
**Files:**
- Create: `crates/adk-rs-fluent/src/live.rs`
- Modify: `crates/adk-rs-fluent/src/lib.rs`
- Modify: `crates/adk-rs-fluent/src/prelude.rs` (via lib.rs prelude block)
...
In `crates/adk-rs-fluent/src/lib.rs`:
```rust
pub mod live;
```
...
Check if `bytes` is in adk-rs-fluent's Cargo.toml; if not, add it.

**Step 4: Build and test**

Run: `cargo build -p adk-rs-fluent && cargo test -p adk-rs-fluent`

**Step 5: Commit**

```
feat(adk-rs-fluent): add Live::builder() fluent API for callback-driven sessions
```
...
|------|-------|--------|---------|
| `crates/rs-genai/src/session/mod.rs` | L0 | Modify | Add SendVideo, UpdateInstruction commands + trait methods |
| `crates/rs-genai/src/transport/codec.rs` | L0 | Modify | Encode new commands to wire format |
| `crates/rs-adk/src/agent_session.rs` | L1 | Modify | Add send_video, update_instruction forwarding |
| `crates/rs-adk/src/live/mod.rs` | L1 | Create | Live session module |
| `crates/rs-adk/src/live/callbacks.rs` | L1 | Create | EventCallbacks registry |
| `crates/rs-adk/src/live/processor.rs` | L1 | Create | Two-lane event processor |
| `crates/rs-adk/src/live/builder.rs` | L1 | Create | LiveSessionBuilder |
| `crates/rs-adk/src/live/handle.rs` | L1 | Create | LiveHandle |
