...
| Primitive | L0 (rs-genai) | L1 (rs-adk) | L2 (adk-rs-fluent) |
|-----------|---------------|-------------|---------------------|
...
**L0 (rs-genai)**: State is implicit. `SessionConfig` is consumed at connection
...
**L1 (rs-adk)**: `State` is a `DashMap<String, Value>` with typed get/set and
...
**L2 (adk-rs-fluent)**: Fluent builders expose state through closures:
...
**L0 (rs-genai)**: Context is modeled as `Content` (role + parts) and `Part`
...
**L1 (rs-adk)**: `TranscriptBuffer` accumulates input/output transcripts and
...
**L2 (adk-rs-fluent)**: The `C::` composition module provides context engineering:
...
**L0 (rs-genai)**: Tools are declared via `FunctionDeclaration` (name, description,
...
**L1 (rs-adk)**: `ToolDispatcher` routes `FunctionCall`s to `ToolFunction`
...
**L2 (adk-rs-fluent)**: Tool registration through builder methods:
...
**L0 (rs-genai)**: System instruction set once via `SessionConfig::system_instruction()`.
...
**L1 (rs-adk)**: Three instruction mechanisms:
...
**L2 (adk-rs-fluent)**: Builder methods:
...
**L0 (rs-genai)**: No middleware system. Extension via traits:
...
**L1 (rs-adk)**: `Middleware` trait for agent execution lifecycle:
...
**L2 (adk-rs-fluent)**: The `M::` composition module provides middleware primitives:
