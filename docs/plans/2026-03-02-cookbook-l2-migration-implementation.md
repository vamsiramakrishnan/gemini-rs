...
**Tech Stack:** `gemini-adk-fluent-rs` (L2 fluent builder), `gemini-adk-rs` (L1 modules: PhaseMachine, WatcherRegistry, TurnExtractor, ComputedRegistry), `gemini-genai-rs` (L0 wire protocol)

**Design doc:** `docs/plans/2026-03-02-cookbook-l2-migration-design.md`

---

### Task 1: Add gemini-adk-fluent-rs dependency and shared helpers

**Files:**
- Modify: `apps/gemini-adk-web-rs/Cargo.toml`
- Modify: `apps/gemini-adk-web-rs/src/apps/mod.rs`

**Step 1: Add gemini-adk-fluent-rs to Cargo.toml**

In `apps/gemini-adk-web-rs/Cargo.toml`, add after the `gemini-adk-rs` line:
...
In `apps/gemini-adk-web-rs/src/apps/mod.rs`, add a shared `resolve_voice` function (currently duplicated in voice_chat, playbook, guardrails, support, all_config). Add this after the `ConversationBuffer` impl:
...
```
feat(examples): add gemini-adk-fluent-rs dependency and shared resolve_voice
```
...
Run: `cargo check -p gemini-genai-ui && cargo test -p gemini-genai-ui`
...
**Files:**
- Create: `apps/gemini-adk-web-rs/src/apps/extractors.rs`
- Modify: `apps/gemini-adk-web-rs/src/apps/mod.rs` (add `pub mod extractors;`)
...
In `apps/gemini-adk-web-rs/src/apps/mod.rs`, add after the existing module declarations:
...
4. Add necessary imports: `use std::sync::Arc;`, `use gemini_adk_fluent_rs::prelude::*;`, `use super::extractors::RegexExtractor;`
...
4. Add: `use std::sync::Arc;`, `use gemini_adk_fluent_rs::prelude::*;`, `use super::extractors::RegexExtractor;`
...
**Files:**
- Modify: `apps/gemini-adk-web-rs/src/apps/mod.rs` (potentially remove ConversationBuffer if unused)

**Step 1: Check if ConversationBuffer is still used**

Search for `ConversationBuffer` in `apps/gemini-adk-web-rs/src/apps/`. If no app imports it anymore, remove the struct from `mod.rs`. Note: the playbook test `conversation_buffer_limits_turns` and guardrails test `conversation_buffer_limits` test this struct directly, so check if those tests still exist.
...
Run: `cargo build -p gemini-genai-ui && cargo test -p gemini-genai-ui`
