...
**Tech Stack:** `adk-rs-fluent` L2 API (`Live::builder()`), `rs-adk` (State, TurnExtractor, BaseLlm, GeminiLlm), `rs-genai` (FunctionCall, FunctionResponse, SessionConfig, SessionEvent), `schemars` (LLM extraction schema), `serde/serde_json`, `regex`, `base64`, `tokio`, `async-trait`, `tracing`.
...
Run: `cargo check -p rs-genai-ui 2>&1`
...
Run: `cargo test -p rs-genai-ui -- debt_collection 2>&1`
    ...
    async fn handle_session(
        ...
        let handle = Live::builder()
            ...
            .on_tool_call(move |calls| {
                ...
                async move {
                    let responses: Vec<rs_genai::prelude::FunctionResponse> = calls
                        .iter()
                        .map(|call| {
...
**Important**: This step may require adjusting import paths or type signatures based on the exact API. If `SessionEvent` is not in scope, add `use rs_genai::session::SessionEvent;`. If `FunctionResponse` isn't in `rs_genai::prelude`, use `rs_genai::protocol::types::FunctionResponse`. Adapt as needed — the exact types are documented in the design doc.

**Step 3: Run all tests**

Run: `cargo test -p rs-genai-ui 2>&1`
...
Run: `cargo check -p rs-genai-ui 2>&1 | grep debt_collection`
