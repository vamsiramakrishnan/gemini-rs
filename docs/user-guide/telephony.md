...
`TwilioCall::attach` (~100 lines) is the reference: it owns no socket,
speaks its platform's dialect at the edges, and delegates everything else
to the shared components. A new connector should look like it.
...
| DTMF | ✓ (`telephony:dtmf*` state) | ✓ (RFC 4733, same state keys) |
