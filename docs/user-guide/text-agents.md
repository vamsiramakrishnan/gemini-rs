...
```rust,ignore
use rs_adk::text::LlmTextAgent;
use rs_adk::llm::GeminiLlm;
...
```rust,ignore
use rs_adk::text::SequentialTextAgent;
...
```rust,ignore
use rs_adk::text::ParallelTextAgent;
...
```rust,ignore
use rs_adk::text::{RouteTextAgent, RouteRule};
...
```rust,ignore
use rs_adk::text::FallbackTextAgent;
...
```rust,ignore
use rs_adk::text::MapOverTextAgent;
...
```rust,ignore
use rs_adk::text::TimeoutTextAgent;
...
```rust,ignore
use rs_adk::text::{DispatchTextAgent, JoinTextAgent, TaskRegistry};
...
If you use the `adk-rs-fluent` crate (L2), you get operator syntax for composing agents:
...
```rust,ignore
use adk_rs_fluent::patterns::{review_loop, cascade, fan_out_merge, supervised};
