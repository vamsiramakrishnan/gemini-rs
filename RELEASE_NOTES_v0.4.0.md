...
apps/gemini-adk-web/      interactive web UI (formerly cookbooks/ui/)
                    apps/gemini-adk-api (new — headless REST API)
                    apps/gemini-adk-server (new — shared server library)
                    tools/gemini-adk-cli/      (new — CLI tool)
```

### `gemini-adk-cli` — Full CLI Tool
...
Five evaluators for comprehensive agent testing, all in `gemini-adk`:
...
- **`gemini-adk-api`** — Standalone REST API server for headless agent deployments, suitable for production use behind a load balancer
- **`gemini-adk-server`** — Shared library extracting agent loading, REST handlers, and session management so that `gemini-adk-web`, `gemini-adk-api`, and `gemini-adk-cli` share the same backend logic

### Production Features

- **Session backends**: Postgres and Vertex AI session persistence (`gemini-adk`)
...
- **Config-driven construction**: `AgentBuilder::from_config()` in `gemini-adk-fluent`
...
|-------|---------|-----------|
| `gemini-live` | 0.4.0 | [crates.io/crates/gemini-live](https://crates.io/crates/gemini-live) |
| `gemini-adk` | 0.4.0 | [crates.io/crates/gemini-adk](https://crates.io/crates/gemini-adk) |
| `gemini-adk-fluent` | 0.4.0 | [crates.io/crates/gemini-adk-fluent](https://crates.io/crates/gemini-adk-fluent) |
| `gemini-adk-cli` | 0.4.0 | [crates.io/crates/gemini-adk-cli](https://crates.io/crates/gemini-adk-cli) |
...
Update your `Cargo.toml` dependencies from `0.1.0` to `0.4.0`. No breaking API changes — this is an additive release. The workspace layout uses `examples/` for standalone examples and `apps/gemini-adk-web/` for the interactive web UI.

## What's Changed (commits)

- `376d975` feat: add missing ADK capabilities and restructure workspace to upstream convention
- `7f6ed8b` feat: consolidate server code into gemini-adk-server shared library
- `0f868d3` docs: add comprehensive READMEs for gemini-adk-web, gemini-adk-api, and gemini-adk-cli
- `7428cdb` chore: add gemini-adk-server to workspace members
- `2dedecb` chore: bump all crate versions to 0.4.0
- `dec514d` docs: add v0.4.0 changelog and update release workflow for gemini-adk-cli
