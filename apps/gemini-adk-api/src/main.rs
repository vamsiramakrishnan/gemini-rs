//! gemini-adk-api — Standalone headless REST API server for ADK agents.
//!
//! Thin wrapper around `gemini-adk-server`. Auto-discovers agents from the
//! current directory (both `agent.json` and `agent.toml` formats) and
//! serves them via REST endpoints.
//!
//! ```bash
//! cargo run -p gemini-adk-api
//! ADK_API_PORT=8080 cargo run -p gemini-adk-api
//! ```
