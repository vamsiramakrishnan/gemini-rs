...
async fn main() {
    ...
    let mut agent_registry = gemini_adk_server::AgentRegistry::new();
    let dir = std::env::current_dir().unwrap_or_default();
    agent_registry.discover(&dir);
    let server_state = gemini_adk_server::ServerState::new(agent_registry);
...
async fn init_telemetry() -> (
    gemini_live::telemetry::TelemetryGuard,
