...
pub async fn run_agent(
    ...
    // TODO: Execute agent via gemini-adk Runner and collect real events.
    let response_text = format!(
        "[{}] (placeholder — wire up gemini_adk::Runner for real execution)",
        agent.name
    );
...
pub async fn run_eval(Json(req): Json<EvalRunRequest>) -> impl IntoResponse {
    // TODO: Wire up to gemini_adk::evaluation
    Json(serde_json::json!({
        "agent": req.agent,
        "status": "submitted",
