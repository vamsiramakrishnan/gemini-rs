...
fn main() {
    ...
    println!(
        "  customer_name: {:?}",
        state.get::<String>("customer_name")
    );
    ...
    let unproduced_fixed: Vec<_> = fixed_violations
        .iter()
        .filter(|v| matches!(v, ContractViolation::UnproducedKey { .. }))
        .collect();

    let duplicates_fixed: Vec<_> = fixed_violations
        .iter()
        .filter(|v| matches!(v, ContractViolation::DuplicateWrite { .. }))
        .collect();
    ...
    if let Composable::Pipeline(p) = &pipeline {
        ...
        for (i, step) in p.steps.iter().enumerate() {
            match step {
                ...
                Composable::Branch(_) => println!("    Step {i}: Branch"),
