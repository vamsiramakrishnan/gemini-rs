...
fn main() {
    ...
    for (i, entry) in all_tools.entries.iter().enumerate() {
        let name = match entry {
            ...
            gemini_adk_fluent_rs::compose::tools::ToolCompositeEntry::BuiltIn(_) => {
                "(built-in)".to_string()
            }
            gemini_adk_fluent_rs::compose::tools::ToolCompositeEntry::Mock { name, .. } => {
                format!("{} (mock)", name)
            }
            _ => "(other)".to_string(),
        };
    ...
    let tool_a: std::sync::Arc<dyn gemini_adk_rs::ToolFunction> = std::sync::Arc::new(
        gemini_adk_rs::SimpleTool::new("tool_a", "Tool A", None, |_| async { Ok(json!(null)) }),
    );
    let tool_b: std::sync::Arc<dyn gemini_adk_rs::ToolFunction> = std::sync::Arc::new(
        gemini_adk_rs::SimpleTool::new("tool_b", "Tool B", None, |_| async { Ok(json!(null)) }),
    );
