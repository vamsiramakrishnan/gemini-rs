...
//! # Callback Modes
//!
//! Control-lane callbacks support two execution modes via [`gemini_adk::live::CallbackMode`]:
//!
//! - **Default methods** (e.g., `.on_turn_complete()`) → [`gemini_adk::live::CallbackMode::Blocking`]
//! - **`_concurrent` methods** (e.g., `.on_turn_complete_concurrent()`) → [`gemini_adk::live::CallbackMode::Concurrent`]
//!
...
use gemini_adk::live::needs::RepairConfig;
use gemini_adk::live::persistence::SessionPersistence;
use gemini_adk::live::steering::{ContextDelivery, SteeringMode};
use gemini_adk::live::{
    ComputedRegistry, EventCallbacks, InstructionModifier, Phase, TemporalRegistry,
    ToolExecutionMode, WatcherRegistry,
};
use gemini_adk::llm::BaseLlm;
...
pub(crate) struct DeferredAgentTool {
    ...
    pub(crate) agent: Arc<dyn gemini_adk::text::TextAgent>,
}
...
mod tests {
    ...
    fn builder_with_extraction_compiles() {
        use gemini_adk::llm::{BaseLlm, LlmError, LlmRequest, LlmResponse};
    ...
    fn builder_with_background_tools_compiles() {
        use gemini_adk::live::DefaultResultFormatter;
