...
use gemini_adk::live::CallbackMode;
...
impl Live {
    ...
    where
        F: Fn(Vec<FunctionResponse>, gemini_adk::State) -> Fut + Send + Sync + 'static,
    ...
    where
        F: Fn(gemini_adk::State, Arc<dyn gemini_live::session::SessionWriter>) -> Fut
            + Send
            + Sync
            + 'static,
    ...
    where
        F: Fn(Arc<dyn gemini_live::session::SessionWriter>) -> Fut + Send + Sync + 'static,
        Fut: Future<Output = ()> + Send + 'static,
    {
