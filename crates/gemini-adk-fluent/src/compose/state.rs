...
impl S {
    ...
    /// ```
    pub fn is_set(key: &str) -> impl Fn(&gemini_adk::State) -> bool + Send + Sync + 'static {
        let key = key.to_string();
        move |s: &gemini_adk::State| s.contains(&key)
    }
    ...
    /// ```
    pub fn is_true(key: &str) -> impl Fn(&gemini_adk::State) -> bool + Send + Sync + 'static {
        let key = key.to_string();
        move |s: &gemini_adk::State| s.get::<bool>(&key).unwrap_or(false)
    }
    ...
    pub fn eq(
        key: &str,
        expected: &str,
    ) -> impl Fn(&gemini_adk::State) -> bool + Send + Sync + 'static {
        let key = key.to_string();
        let expected = expected.to_string();
        move |s: &gemini_adk::State| {
            s.get::<String>(&key)
                .map(|v| v == expected)
                .unwrap_or(false)
        }
    ...
    pub fn one_of(
        ...
        move |s: &gemini_adk::State| s.get::<String>(&key).is_some_and(|v| values.contains(&v))
    }
