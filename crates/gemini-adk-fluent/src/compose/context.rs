            ...
            history
                ...
                .filter(|c| {
                    c.parts.iter().any(|p| match p {
                        gemini_live::prelude::Part::Text { text } => owned_names
                            .iter()
...
mod tests {
    ...
    fn from_state_prepends_context() {
        ...
        if let gemini_live::prelude::Part::Text { text } = &result[0].parts[0] {
    ...
    fn fit_truncates_with_marker() {
        ...
        let has_marker = result.iter().any(|c| {
            c.parts.iter().any(|p| match p {
                gemini_live::prelude::Part::Text { text } => text.contains("truncated"),
                _ => false,
            })
        });
        assert!(has_marker);
