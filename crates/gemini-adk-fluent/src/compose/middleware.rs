...
use gemini_adk::context::AgentEvent;
use gemini_adk::error::{AgentError, ToolError};
use gemini_adk::middleware::{LatencyMiddleware, LogMiddleware, Middleware};
...
impl M {
    ...
    pub fn retry(max_retries: u32) -> MiddlewareComposite {
        MiddlewareComposite::new(Arc::new(gemini_adk::middleware::RetryMiddleware::new(
            max_retries,
        )))
    }
    ...
    pub fn before_model(
        f: impl Fn(&gemini_adk::llm::LlmRequest) -> Result<(), String> + Send + Sync + 'static,
    ) -> MiddlewareComposite {
    ...
    pub fn after_model(
        f: impl Fn(&gemini_adk::llm::LlmRequest, &gemini_adk::llm::LlmResponse) -> Result<(), String>
            + Send
            + Sync
            + 'static,
    ) -> MiddlewareComposite {
...
impl Middleware for TraceMiddleware {
    ...
    async fn before_agent(
        &self,
        ctx: &gemini_adk::context::InvocationContext,
    ) -> Result<(), AgentError> {
        let sid = ctx.session_id.as_deref().unwrap_or("unknown");
        gemini_adk::telemetry::logging::log_agent_started(sid, 0);
        Ok(())
    }

    async fn before_tool(&self, call: &FunctionCall) -> Result<(), AgentError> {
        gemini_adk::telemetry::logging::log_tool_dispatch("fluent", &call.name, "function");
        Ok(())
    }
    ...
    async fn after_tool(
        ...
        gemini_adk::telemetry::logging::log_tool_result("fluent", &call.name, true, 0.0);
        Ok(())
    }

    async fn on_tool_error(&self, call: &FunctionCall, _err: &ToolError) -> Result<(), AgentError> {
        gemini_adk::telemetry::logging::log_tool_result("fluent", &call.name, false, 0.0);
        Ok(())
    }

    async fn on_error(&self, err: &AgentError) -> Result<(), AgentError> {
        gemini_adk::telemetry::logging::log_agent_error("fluent", &err.to_string());
        Ok(())
    }
}
...
pub struct CacheMiddleware {
    cache: parking_lot::Mutex<std::collections::HashMap<u64, gemini_adk::llm::LlmResponse>>,
}
...
impl Middleware for CacheMiddleware {
    ...
    async fn before_model(
        &self,
        request: &gemini_adk::llm::LlmRequest,
    ) -> Result<Option<gemini_adk::llm::LlmResponse>, AgentError> {
    ...
    async fn after_model(
        &self,
        request: &gemini_adk::llm::LlmRequest,
        response: &gemini_adk::llm::LlmResponse,
    ) -> Result<Option<gemini_adk::llm::LlmResponse>, AgentError> {
        use std::hash::{Hash, Hasher};
        let mut hasher = std::collections::hash_map::DefaultHasher::new();
        format!("{:?}", request).hash(&mut hasher);
...
impl Middleware for SampleMiddleware {
    ...
    async fn before_model(
        &self,
        _request: &gemini_adk::llm::LlmRequest,
    ) -> Result<Option<gemini_adk::llm::LlmResponse>, AgentError> {
        use std::hash::{Hash, Hasher};
...
impl Middleware for MetricsMiddleware {
    ...
    async fn before_agent(
        &self,
        _ctx: &gemini_adk::context::InvocationContext,
    ) -> Result<(), AgentError> {
        self.request_count
            .fetch_add(1, std::sync::atomic::Ordering::SeqCst);
        Ok(())
...
struct BeforeModelMiddleware {
    #[allow(clippy::type_complexity)]
    handler: Arc<dyn Fn(&gemini_adk::llm::LlmRequest) -> Result<(), String> + Send + Sync>,
}
...
struct AfterModelMiddleware {
    #[allow(clippy::type_complexity)]
    handler: Arc<
        dyn Fn(&gemini_adk::llm::LlmRequest, &gemini_adk::llm::LlmResponse) -> Result<(), String>
            + Send
            + Sync,
    >,
}
