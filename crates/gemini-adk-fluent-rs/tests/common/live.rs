...
impl Observed {
    ...
    /// Wait for one turn boundary and report what happened, whether or not the
    /// model spoke.
    ///
    /// [`try_answer`](Self::try_answer) waits for *speech*, which is right when
    /// a test is asserting on an answer and wrong when driving a conversation:
    /// a turn in which the model only calls a tool completes with an empty
    /// transcript, so `try_answer` sails past the real boundary and blocks
    /// until the next one — which will not arrive until the caller speaks
    /// again. Driving a script that way produces 90-second "turns", a caller
    /// talking over a model that is still working, and a transcript where three
    /// replies arrive at once.
    ///
    /// Returns the text spoken since `mark` (possibly empty) once the turn ends,
    /// or `None` if the turn never completed.
    pub async fn try_turn(&self, mark: Mark) -> Option<String> {
        let deadline = Instant::now() + TURN_TIMEOUT;
        let remaining = deadline.saturating_duration_since(Instant::now());
        if tokio::time::timeout(remaining, self.turn.notified())
            .await
            .is_err()
        {
            return None;
        ...
        Some(self.spoken_since(mark))
    }

    /// Let the model finish before the caller speaks again.
    ///
    /// Audio keeps arriving after the turn boundary that carried it. Sending
    /// the next utterance immediately is barge-in — technically valid, and not
    /// what a scripted happy path is trying to measure.
    pub async fn settle(&self) {
        let mut last = self.audio_bytes.load(Ordering::Relaxed);
        for _ in 0..20 {
            tokio::time::sleep(Duration::from_millis(250)).await;
            let now = self.audio_bytes.load(Ordering::Relaxed);
            if now == last {
                return;
            }
            last = now;
