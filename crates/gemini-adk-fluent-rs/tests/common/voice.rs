...
/// FNV-1a, 64-bit — inlined so this harness carries no dependency on the
/// memory crate. Same function as `gemini_memory_rs::core::stable_hash`, which
/// matters only in that cache files are interchangeable between the two.
