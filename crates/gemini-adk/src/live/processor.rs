...
use gemini_live::prelude::{SessionEvent, SessionPhase, UsageMetadata};
...
pub(crate) enum ControlEvent {
    ...
    SessionResumeUpdate(gemini_live::session::ResumeInfo),
    Error(String),
    ...
    async fn background_tool_sends_ack_immediately() {
        ...
        impl SessionWriter for RecordingWriter {
            ...
            async fn send_client_content(
                &self,
                _content: Vec<gemini_live::prelude::Content>,
                _turn_complete: bool,
            ) -> Result<(), gemini_live::session::SessionError> {
                Ok(())
            }
