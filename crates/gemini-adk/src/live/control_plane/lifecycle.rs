...
use gemini_live::prelude::SessionEvent;
...
pub(in crate::live) async fn handle_turn_complete(
    ...
    let mut context_buffer: Vec<gemini_live::prelude::Content> = Vec::new();
        ...
        if let Some(ref pm) = phase_machine {
            ...
            if let Some(tools) = machine.active_tools() {
                ...
                if changed {
                    ...
                    context_buffer.push(gemini_live::prelude::Content::model(format!(
    ...
    if let Some(instruction) = resolved_instruction {
        match control_plane.steering_mode {
            ...
            SteeringMode::ContextInjection => {
                context_buffer.push(gemini_live::prelude::Content::model(instruction));
            }
        }
    }
