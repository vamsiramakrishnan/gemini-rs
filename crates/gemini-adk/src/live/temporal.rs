...
use gemini_live::session::{SessionEvent, SessionWriter};
...
mod tests {
    ...
    impl SessionWriter for MockWriter {
        async fn send_audio(&self, _: Vec<u8>) -> Result<(), gemini_live::session::SessionError> {
            Ok(())
        }
        async fn send_text(&self, _: String) -> Result<(), gemini_live::session::SessionError> {
            Ok(())
        }
        async fn send_tool_response(
            &self,
            _: Vec<gemini_live::protocol::FunctionResponse>,
        ) -> Result<(), gemini_live::session::SessionError> {
            Ok(())
        }
        async fn send_client_content(
            &self,
            _: Vec<gemini_live::protocol::Content>,
            _: bool,
        ) -> Result<(), gemini_live::session::SessionError> {
            Ok(())
        }
        async fn send_video(&self, _: Vec<u8>) -> Result<(), gemini_live::session::SessionError> {
            Ok(())
        }
        ...
        async fn disconnect(&self) -> Result<(), gemini_live::session::SessionError> {
            Ok(())
        }
    }
