...
pub(in crate::live) async fn run_control_lane(
    ...
    // The lane advances it after handling `TurnComplete`, so everything it
    // dequeued before the boundary — tool calls, transcripts, the boundary
    // itself — lands on the turn it belongs to.
    let mut turn = crate::live::turn_trace::TurnTrace::new();

    while let Some(event) = rx.recv().await {
        let boundary = matches!(event, ControlEvent::TurnComplete);
        let span = turn.span();
        async {
            match event {
                ...
                ControlEvent::TurnComplete => {
                    ...
                    tracing::info!(
                        input_chars = accumulated_input.len(),
                        output_chars = accumulated_output.len(),
                        "turn complete"
                    );
