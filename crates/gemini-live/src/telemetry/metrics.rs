...
pub fn record_session_connected() {
    metrics::counter!("gemini_live_connections_total").increment(1);
    metrics::gauge!("gemini_live_sessions_active").increment(1.0);
}

/// Record a session disconnection.
#[cfg(feature = "metrics")]
pub fn record_session_disconnected() {
    metrics::gauge!("gemini_live_sessions_active").decrement(1.0);
}

/// Record audio send latency.
#[cfg(feature = "metrics")]
pub fn record_audio_latency(latency_ms: f64) {
    metrics::histogram!("gemini_live_audio_latency_ms").record(latency_ms);
}

/// Record time from end-of-speech to first model response.
#[cfg(feature = "metrics")]
pub fn record_response_latency(latency_ms: f64) {
    metrics::histogram!("gemini_live_response_latency_ms").record(latency_ms);
}

/// Record current jitter buffer depth.
#[cfg(feature = "metrics")]
pub fn record_jitter_depth(depth_ms: f64) {
    metrics::gauge!("gemini_live_jitter_buffer_depth_ms").set(depth_ms);
}

/// Record a jitter buffer underrun.
#[cfg(feature = "metrics")]
pub fn record_jitter_underrun() {
    metrics::counter!("gemini_live_jitter_underruns_total").increment(1);
}

/// Record a tool call execution.
#[cfg(feature = "metrics")]
pub fn record_tool_call(function_name: &str, duration_ms: f64) {
    metrics::counter!("gemini_live_tool_calls_total", "function" => function_name.to_string())
        .increment(1);
    metrics::histogram!("gemini_live_tool_call_duration_ms", "function" => function_name.to_string())
        .record(duration_ms);
}
...
pub fn record_vad_event(event: &str) {
    metrics::counter!("gemini_live_vad_events_total", "event" => event.to_string()).increment(1);
}

/// Record a reconnection attempt.
#[cfg(feature = "metrics")]
pub fn record_reconnection() {
    metrics::counter!("gemini_live_reconnections_total").increment(1);
}

/// Record WebSocket bytes sent.
#[cfg(feature = "metrics")]
pub fn record_ws_bytes_sent(bytes: u64) {
    metrics::counter!("gemini_live_ws_bytes_sent_total").increment(bytes);
}

/// Record WebSocket bytes received.
#[cfg(feature = "metrics")]
pub fn record_ws_bytes_received(bytes: u64) {
    metrics::counter!("gemini_live_ws_bytes_received_total").increment(bytes);
}
...
pub fn record_http_request(method: &str, status: u16, duration_ms: f64) {
    metrics::counter!(
        "gemini_live_http_requests_total",
        "method" => method.to_string(),
        "status" => status.to_string()
    )
    .increment(1);
    metrics::histogram!(
        "gemini_live_http_request_duration_ms",
        "method" => method.to_string()
    )
    .record(duration_ms);
}
